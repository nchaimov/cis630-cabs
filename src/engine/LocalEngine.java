...
public class LocalEngine extends Engine {
	...
	private void sendCells() {
		/*
		 * The Server will be have more cells than the rest of the Engines, but
		 * that is OK.
		 */
	...
	public static void main(String[] args) {
		...
		while (i < args.length && args[i].startsWith("--")) {
			...
			if (arg.equals("--help")) {
				System.out
						.println("Usage: LocalEngine: [--isClient] IPAddress [--setSize] width height "
								+ "[--port] portNum");
				System.exit(0);
			...
			// note: if --setSize is called by the client it probably shouldn't
			// change
			// the size of it's screen. Not sure how to handle this with shared
			// code.
			...
			// not sure if we even want to include this with the random porting
			// idea
